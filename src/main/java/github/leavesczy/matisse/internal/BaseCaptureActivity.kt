...
internal abstract class BaseCaptureActivity : AppCompatActivity() {
    ...
    protected fun requestTakePicture() {
        if (captureStrategy.shouldRequestWriteExternalStoragePermission(context = applicationContext)) {
            if (permissionGranted(context = this, permission = Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                requestCameraPermissionIfNeed()
            } else {
                showPermissionRationaleDialog(
                    message = getString(R.string.matisse_write_external_storage_permission_rationale),
                    onConfirm = {
    ...
    private fun requestCameraPermissionIfNeed() {
        lifecycleScope.launch(context = Dispatchers.Main.immediate) {
            ...
            if (requirePermissionToTakePhotos) {
                showPermissionRationaleDialog(
                    message = getString(R.string.matisse_camera_permission_rationale),
                    onConfirm = {
    ...
    protected fun showPermissionRationaleDialog(
        message: String,
        onConfirm: () -> Unit,
        onCancel: () -> Unit
    ) {
        AlertDialog.Builder(this)
            .setTitle(R.string.matisse_permission_rationale_title)
            .setMessage(message)
            .setPositiveButton(R.string.matisse_permission_rationale_confirm) { _, _ -> onConfirm() }
            .setNegativeButton(R.string.matisse_permission_rationale_cancel) { _, _ -> onCancel() }
            .show()
    }
